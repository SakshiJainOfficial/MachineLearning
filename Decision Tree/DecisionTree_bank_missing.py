...
for i in dataset.columns:
    if dataset[i].dtype!='int64':
        if np.sum([dataset[i].values == 'unknown']) > 0:
            for j in dataset[i].value_counts().keys():
                if j != 'unknown':
                    dataset[i] = dataset[i].replace({'unknown': j})
                    break
for i in testdataset.columns:
    if testdataset[i].dtype!='int64':
        if np.sum([testdataset[i].values == 'unknown']) > 0:
            for j in testdataset[i].value_counts().keys():
                if j != 'unknown':
                    testdataset[i] = testdataset[i].replace({'unknown': j})
                    break


#decision tree depth - 1 to 6

print("Train Errors(Unknown as missing):")
print("             Gini Index     Entropy     Majority Error    Average Error")
for i in range(1,max_depth+1):
    decisiontree_entropy_missing = ID3(i, 0)
    decisiontree_gini_missing = ID3(i, 1)
    decisiontree_me_missing = ID3(i, 2)
    decisiontree_entropy_missing.fit(dataset,copy.deepcopy(attribute),0)
    decisiontree_gini_missing.fit(dataset, copy.deepcopy(attribute), 0)
    decisiontree_me_missing.fit(dataset, copy.deepcopy(attribute), 0)
    predictedlabels_train_me = decisiontree_me_missing.testdataset(dataset)
    predictedlabels_train_gini = decisiontree_gini_missing.testdataset(dataset)
    predictedlabels_train_ent = decisiontree_entropy_missing.testdataset(dataset)
...
print("Test Errors(unknown as missing):")
print("             Gini Index     Entropy     Majority Error    Average Error")
for i in range(1, max_depth + 1):
    decisiontree_entropy_test_missing = ID3(i, 0)
    decisiontree_gini_test_missing = ID3(i, 1)
    decisiontree_me_test_missing = ID3(i, 2)
    decisiontree_entropy_test_missing.fit(testdataset, copy.deepcopy(attribute), 0)
    decisiontree_gini_test_missing.fit(testdataset, copy.deepcopy(attribute), 0)
    decisiontree_me_test_missing.fit(testdataset, copy.deepcopy(attribute), 0)
    predictedlabels_test_me = decisiontree_me_test_missing.testdataset(testdataset)
    predictedlabels_test_gini = decisiontree_gini_test_missing.testdataset(testdataset)
    predictedlabels_test_ent = decisiontree_entropy_test_missing.testdataset(testdataset)
